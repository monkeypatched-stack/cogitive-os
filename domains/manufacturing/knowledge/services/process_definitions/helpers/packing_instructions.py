...
async def get_all(
    ...
    cursor = (
        db[COLLECTION]
        .find(query)
        .sort([("process_definition_id", 1), ("revision", -1), ("packing_instruction_id", 1)])
...
async def get_by_id(db: AsyncIOMotorDatabase, packing_instruction_id: str) -> Optional[dict]:
    doc = await db[COLLECTION].find_one({"packing_instruction_id": packing_instruction_id})
...
async def update(
    ...
    existing = await db[COLLECTION].find_one({"packing_instruction_id": packing_instruction_id})
    if not existing:
        return None
...
async def delete(db: AsyncIOMotorDatabase, packing_instruction_id: str) -> bool:
    result = await db[COLLECTION].delete_one({"packing_instruction_id": packing_instruction_id})
