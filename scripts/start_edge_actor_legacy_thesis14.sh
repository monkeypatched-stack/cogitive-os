#!/bin/bash
# LEGACY (Thesis 14 prototype) — CognitiveOS Edge Actor Startup Script
#
# STATUS: this boots the standalone, disconnected EdgeActor prototype
# (src/sync/edge_server.py/edge_actor.py) — tabular-RL only, no real
# ActorIdentity/governance/capabilities. Kept unchanged for backward
# compatibility with tests/unit/test_edge_cloud.py. For a real edge
# deployment of the actual governed CognitiveActor, use
# scripts/start_actor.sh --node-class edge instead (see
# docs/ACTOR_ARTIFACT.md / docs/CLOUD_EDGE_ACTOR_ARCHITECTURE.md).
# scripts/start_edge_actor.sh (no suffix) now points at that real path.
#
